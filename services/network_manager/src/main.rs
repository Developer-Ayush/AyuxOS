use libaipc::{AipcMessage, NetworkRequest, NetworkResponse, create_listener, AipcClient, AipcHeader, AipcEnvelope, MessageType, AIPC_VERSION, HalRequest, HalResponse};
use std::fs;
use std::io::{self};
...
impl NetworkManager {
    ...
    fn get_interface_config(&self, name: &str) -> NetworkResponse {
        ...
        let mac = fs::read_to_string(format!("{}/address", base_path)).unwrap_or_default().trim().to_string();
    ...
    fn handle_request(&self, request: NetworkRequest) -> NetworkResponse {
        match request {
            ...
            NetworkRequest::ConfigureInterface { name, up, dhcp } => self.configure_interface(&name, up, dhcp),
        }
    }
        ...
        match request {
            HalRequest::DiscoveryScan => {
                ...
                match discovery.scan() {
                    Ok(devices) => HalResponse::DiscoveryResult(devices.into_iter().map(|d| format!("{} [{}] at {}", d.name, d.class, d.path)).collect()),
...
fn main() {
    println!("[Network Manager] Starting...");

    let manager = NetworkManager::new();

    // Initial loopback configuration
    println!("[Network Manager] Configuring loopback...");
    let _ = Command::new("ip").args(&["link", "set", "lo", "up"]).status();

    let listener = create_listener(NETWORK_SOCKET_PATH).expect("Failed to create network socket");
    println!("[Network Manager] Listening on {}", NETWORK_SOCKET_PATH);
